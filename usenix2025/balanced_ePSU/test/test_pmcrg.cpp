...
void pMCRG_test(u32 idx, u32 logNum, u32 numThreads){
    u32 numElements = 1 << logNum;    
    oc::CuckooParam params = oc::CuckooIndex<>::selectParams(numElements, ssp, 0, 3);
    u32 numBins = params.numBins(); // the real num of pMCRG that is used in the whole protocol  
    ...
    pMCRG(idx, numElements, inputSet, pmcrg_out, permutedX0, chl, numThreads);
    ...
    if(idx == 0){
        coproto::sync_wait(chl.send(pmcrg_out));
    }
    else if(idx == 1){
        std::vector<block> pmcrg_out0(numBins);
        coproto::sync_wait(chl.recv(pmcrg_out0));

        u32 count = 0;
        for(u32 i = 0; i < numBins; ++i){
            if(pmcrg_out0[i] == pmcrg_out[i]){
                count += 1;
            }
        }
        if(count == equalNum){
            std::cout << "pMCRG functionality test pass!" << std::endl;
        }
        else{
            std::cout << "pMCRG functionality test fail!" << std::endl;
        }
...
int main(int agrc, char** argv){
    ...
    if (help){
        std::cout << "protocol: permuted membership conditional randomness generation" << std::endl;
