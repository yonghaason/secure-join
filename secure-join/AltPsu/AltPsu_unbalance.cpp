...
namespace secJoin
{
    ...
    Proto AltModPsuSender::run_unbalance(span<block> Y, span<block> X, PRNG &prng, Socket &chl, Socket &socket_gmw)
    {
        MC_BEGIN(Proto, this, Y, X, &prng, &chl,
                i = int{},
                epsilon = double{},
                n = int{},
                m = int{},
        ...
        // std::cout << "sender start okvs.decode\n";
        epsilon = 0.1;
        n = Y.size();
        m = static_cast<int>((1 + epsilon) * n);
        band_length = 128;

        okvs2.Init(n, m, band_length, oc::ZeroBlock);

        decoded.resize(Y.size());
        decoded2.resize(Y.size());
        okvs2.Decode(Y.data(), okvs_encoding.data(), decoded.data());
        okvs2.Decode(Y.data(), okvs_encoding2.data(), decoded2.data());

        // std::cout << "sender end okvs.decode\n";
        ...
        // std::cout << "decoded.size() is " << decoded.size() << '\n';
        // std::cout << "output_2party_wprf.size() is " << output_2party_wprf.size() << '\n';
        ...
        // gmw part
        cir = isZeroCircuit(60);
        
        in.resize(Y.size(), 8);
        for (i = 0; i < Y.size(); i++){
            temp = output_2party_wprf[i]^decoded2[i];
            std::memcpy(&in(i, 0), &temp, 8);
        }
    ...
    Proto AltModPsuReceiver::run_unbalance(span<block> X, span<block> Y, PRNG &prng, Socket &chl, Socket &socket_gmw)
    {
        MC_BEGIN(Proto, this, X, Y, &prng, &chl,
                i = int{},
                epsilon = double{},
                n = int{},
                m = int{},
        ...
        // input_of_gmw.resize(X.size());

        // for(i = 0; i < X.size(); i++)
        //     input_of_gmw[i] = indicator_string;
        ...
        ot_choice.resize(Y.size());
        ot_choice = oc::BitVector(outView.data(), Y.size(), /*copyData=*/false);
