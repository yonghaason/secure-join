...
namespace secJoin
{
    ...
    struct Where{
        ...
        // Can't make Where class statis otherwise the below objects will be shared
        std::vector<WhBundle> mWhBundle;
        std::vector<BinMatrix*> mInputs;
        ...
        void addColInVec(SharedTable& st, u64 gateInputIndex,
            const std::unordered_map<u64, u64>& map);

        void addOutputBundle(oc::BetaCircuit* cd, BetaBundle &c, bool lastOp);

        u64 getMapVal(const std::unordered_map<u64, u64>& map, u64 tag);

        void ArrTypeLessThanCir(u64 inIndex1, u64 inIndex2, SharedTable& st, oc::BetaCircuit* cd, 
            const std::unordered_map<u64, u64>& map, BetaBundle &c, bool lastOp);

        void ArrTypeAddCir(u64 inIndex1, u64 inIndex2, SharedTable& st, oc::BetaCircuit* cd, 
            const std::unordered_map<u64, u64>& map, BetaBundle &c, bool lastOp);

        void ArrTypeEqualsCir(const u64 inIndex1, const u64 inIndex2, SharedTable& st, oc::BetaCircuit* cd, 
            const std::unordered_map<u64, u64>& map, BetaBundle &c, bool lastOp);

        void signExtend(u64 biggerSize, u64 smallerSizeIndex, u64 biggerSizeIndex,
            SharedTable& st, const std::unordered_map<u64, u64>& map);

        void signExtend(BitVector& aa, u64 size, WhType type);
        BinMatrix* signExtend(BinMatrix& in, u64 size, TypeID type);
