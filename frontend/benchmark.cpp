...
void AltModPsu_benchmark(const oc::CLP &cmd) {
  ...
  for (u64 i = 0; i < 1; ++i) {
    ...
    auto p1 = recv.run(recvSet, diffSet, prng, socket[1]);
...
void CorGen_benchmark(const oc::CLP &cmd) {
  ...
  for (u64 trial = 0; trial < trials; ++trial) {
    ...
    for (u64 i = 0; i < std::max<u64>(ole, ot);) {
      ...
      // timer.setTimePoint("start batches " + std::to_string(i) + " ... " +
      ...
      for (u64 b = 0; b < chunk; ++b, ++i) {
        if (i < ole) {
          ...
          while (m < n) {
            ...
            macoro::sync_wait(macoro::when_all_ready(oleReqs[i][0].get(o[0]),
                                                     oleReqs[i][1].get(o[1])));
        ...
        if (i < ot) {
          ...
          OtSend s;
          while (m < n) {
            macoro::sync_wait(macoro::when_all_ready(otRecvReqs[i].get(r),
                                                     otSendReqs[i].get(s)));
            m += r.size();
          }
        }
      }
      // timer.setTimePoint("done batches " + std::to_string(i) + " ... " +
      // std::to_string(i + chunk));
    }
...
void Radix_benchmark(const oc::CLP &cmd) {
  ...
  for (u64 t = 0; t < trials; ++t) {
    ...
    auto r = coproto::sync_wait(coproto::when_all_ready(
        g[0].start(), g[1].start(), s0.genPerm(k[0], d[0], sock[0], prng0),
  ...
  std::cout << "radix n:" << n << ", m:" << m << "  : "
            << std::chrono::duration_cast<std::chrono::milliseconds>(end -
                                                                     begin)
                   .count()
            << "ms " << sock[0].bytesSent() / double(n) << "+"
            << sock[0].bytesReceived() / double(n) << "="
            << (sock[0].bytesSent() + sock[0].bytesReceived()) / double(n)
            << " bytes/eval " << std::endl;
  // std::cout << ole0.mNumBinOle / double(n) << " " << ole1.mNumBinOle /
  // double(n) << " binOle/per" << std::endl;;
  if (cmd.isSet("v")) {
    std::cout << timer << std::endl;
...
void OmJoin_benchmark(const oc::CLP &cmd) {
  ...
  auto r = macoro::sync_wait(macoro::when_all_ready(
      ole0.start(), ole1.start(), join0.join(query0, out[0], prng0, sock[0]),
      join1.join(query1, out[1], prng1, sock[1])));
  auto end = timer.setTimePoint("end");

  std::cout << "OmJoin Ln:" << nL << ", Rn:" << nR << " m:" << keySize
            << "  Ld: " << dL << ", Rd:" << dR << "  ~ "
            << std::chrono::duration_cast<std::chrono::milliseconds>(end -
                                                                     begin)
                   .count()
            << "ms " << sock[0].bytesSent() / double(nL + nR) << "+"
            << sock[0].bytesReceived() / double(nL + nR) << "="
            << (sock[0].bytesSent() + sock[0].bytesReceived()) / double(nL + nR)
            << " bytes/elem " << std::endl;

  if (cmd.isSet("timing")) std::cout << timer << std::endl;
}
...
void OT_benchmark(const oc::CLP &cmd) {
  ...
  try {
    ...
    std::cout << "OT n:" << n << ", " << avgTime << "ms/batch, " << timePer512
              << "ms/512ot" << std::endl;

    std::cout << rTimer << std::endl;

    std::cout << sock[0].bytesReceived() / trials << " "
              << sock[1].bytesReceived() / trials << " bytes per " << std::endl;
...
void F4_benchmark(const oc::CLP &cmd) {
  ...
  try {
    ...
    auto avgTime = time / double(trials);
    auto timePer512 = avgTime / n * 512;
    std::cout << "F4 n:" << n << ", " << avgTime << "ms/batch, " << timePer512
              << "ms/512ot" << std::endl;

    if (cmd.isSet("verbose")) {
      std::cout << rTimer << std::endl;
      std::cout << sock[0].bytesReceived() / trials << " "
                << sock[1].bytesReceived() / trials << " bytes per "
                << std::endl;
    }
  } catch (std::exception &e) {
    std::cout << e.what() << std::endl;
...
void AltMod_compressB_benchmark(const oc::CLP &cmd) {
  ...
  if (cmd.isSet("single")) {
    ...
    oc::block bb;
    for (u64 i = 0; i < n; ++i) bb = bb ^ y.data()[i];

    std::cout
        << "single compressB n:" << n << ", "
...
void AltMod_encodeX_benchmark(const oc::CLP &cmd) {
  ...
  if (cmd.isSet("e1")) {
    for (u64 tt : stdv::iota(0, t)) {
      ...
      std::cout << "expand  n:" << n << ", "
                << std::chrono::duration_cast<std::chrono::milliseconds>(e - b)
                       .count()
                << "ms " << std::endl;
      ;
    }
...
void AltMod_sampleMod3_benchmark(const oc::CLP &cmd) {
  ...
  //}
  {
    ...
    for (u64 k = 0; k < t; ++k) sampleMod3Lookup(prng, msb, lsb);
    auto e = timer.setTimePoint("end");
...
void PprfPerm_benchmark(const oc::CLP &cmd) {
  ...
  auto totalComm =
      sock[0].bytesSent() + sock[0].bytesReceived() + N * 16 * depth * trials;

  std::cout << "PprfPerm n:" << N << ", #ots/per " << (recver.mBaseCount / N)
            << " depth " << depth << " "
            << std::chrono::duration_cast<std::chrono::milliseconds>(end -
                                                                     begin)
                       .count() /
                   double(trials)
            << "ms " << (totalComm) / double(1 << 20) << " MB/N " << std::endl;
